function [Pd1,C1] = decomp_poly_ns_discrete_WMproj(A,B,rs,dv,o,T)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% T:        1 x 1 scalar for the number of time points to look back
% 
% Outputs:
...
% xdot terms
% C1 = repmat([As*CM(:,1), CM(:,2:end)],[1,1,T]);
C1 = repmat(CM,[1,1,T]);
for i = 2:T
    C1(:,:,i) = As*C1(:,:,i-1);
end
C1(:,1,:) = repmat(rs,[1,1,T])/T;
C1 = flip(C1,3);

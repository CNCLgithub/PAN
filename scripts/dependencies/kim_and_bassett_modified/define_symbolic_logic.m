function [x,dx] = define_symbolic_logic(board_params)
%%
...
for i = 1:nwalls
    ...
    if strcmp(wall ,board_params.paddle_pos) % if this wall is the paddle      
         switch wall
             case {"right" "left"} % for vertical paddle we care about paddle y-pos
                x_dist = (abs(x(3)) +     xw)^2;
                y_dist = (abs(x(4)) - paddle)^2;
                
            case {"top" "bottom"} % for horizontal paddle we care about paddle x-pos
                x_dist = (abs(x(3)) - paddle)^2;
                y_dist = (abs(x(4)) +     yw)^2;
         end

         mult_sr = 50; mult_col = 2000;
         ball_dist = ps - ( x_dist + y_dist );
    else
...
% zero out collision detector dynamics for paddle
% if ~paddle_exist
